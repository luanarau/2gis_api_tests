...
class POST_query_Responce():
    ...
    def __init__(self, input_data: dict):
        ...
        if self.status_code != 200:
            self.json_answer = self.answer.text
        else:
    ...
    def validate_status_code(self, result_status: int):
        assert self.status_code == result_status, f'\nWrong status code, expected {result_status}, got {self.status}'
        return self

    def validate_result_status(self, result_status: str):
        assert self.status == result_status, f'\nWrong result status, expected {result_status}, got {self.status}'
        return self
